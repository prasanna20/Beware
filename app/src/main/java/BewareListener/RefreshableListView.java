...
public class RefreshableListView extends FrameLayout {
	...
	private static int itemsFromBottom = 10;
						...
						if(!this.getAdapter().isEmpty())
							if(totalItemCount  == firstVisibleItem+visibleItemCount) {
								if (ListrefreshLister != null)
									Log.i("RefreshListener","into last");
									ListLoadMoreListener.LoadMore(RefreshableListView.this,1);
							     }
			                     else {
								if(FirstItem != firstVisibleItem)
								{
									FirstItem=firstVisibleItem;
									Log.i("RefreshListener", "in position " + firstVisibleItem);
									ListLoadMoreListener.LoadMore(RefreshableListView.this,2);
								}
